...
def load_hybrid_data(data_root: str):
    ...
    X_feat_train = np.load(os.path.join(data_root, "X_feat_train.npy"))
    X_feat_val = np.load(os.path.join(data_root, "X_feat_val.npy"))
...
def main():
    ...
    parser.add_argument("--save_root", type=str, default="./results/hybrid",
                        help="결과 저장 루트")
    ...
    (
        ...
        X_feat_train, X_feat_val, X_feat_test,
        y_train, y_val, y_test
    ) = load_hybrid_data(args.data_root)

    print(f"[INFO] X_seq_train : {X_seq_train.shape}")
    print(f"[INFO] X_feat_train: {X_feat_train.shape}")
    print(f"[INFO] y_train     : {y_train.shape}")

    print(f"[INFO] X_seq_val   : {X_seq_val.shape}")
    print(f"[INFO] X_feat_val  : {X_feat_val.shape}")
    print(f"[INFO] y_val       : {y_val.shape}")

    print(f"[INFO] X_seq_test  : {X_seq_test.shape}")
    print(f"[INFO] X_feat_test : {X_feat_test.shape}")
    print(f"[INFO] y_test      : {y_test.shape}")

    seq_input_dim = X_seq_train.shape[-1]
    feat_input_dim = X_feat_train.shape[-1]

    train_dataset = HybridDataset(X_seq_train, X_feat_train, y_train)
    val_dataset = HybridDataset(X_seq_val, X_feat_val, y_val)
    test_dataset = HybridDataset(X_seq_test, X_feat_test, y_test)
    ...
    exp_name = f"hybrid_gru_h{args.hidden_dim}_l{args.num_layers}"
