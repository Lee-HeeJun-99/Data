...
MODEL_PATH = os.path.join(
    BASE_DIR, "results", "best_model.pt"
)
...
class DeviceRuntime:
    def __init__(
        ...
        self.spark_hf_norm = deque(maxlen=MINI_SERIES_LEN)
        self.spark_arc_flag = deque(maxlen=MINI_SERIES_LEN)
        self.spark_arc_detect = deque(maxlen=MINI_SERIES_LEN)
    ...
    def reset(self):
        ...
        self.spark_hf_norm.clear()
        self.spark_arc_flag.clear()
        self.spark_arc_detect.clear()
        self.spark_t.clear()
    ...
    def _spark_dict(self):
        return {
            ...
            "hf_norm": list(self.spark_hf_norm),
            "arc_flag": list(self.spark_arc_flag),
            "arc_detect": list(self.spark_arc_detect),
    ...
    def push_sensor(self, sensor: Dict[str, Any]):
        ...
        self.spark_hf_norm.append(float(sensor["hf_norm"]))
        self.spark_arc_flag.append(float(sensor["arc_flag"]))
        self.spark_arc_detect.append(float(sensor["arc_detect"]))
...
# =========================
# CSV 재생 + 추론 + DB 저장 + WebSocket + MQTT
# =========================
async def replay_device_csv(device: DeviceRuntime):
    ...
    try:
        ...
        if df[required_cols].isna().any().any():
            ...
            raise ValueError(f"CSV에 NaN 또는 숫자 변환 불가 값이 있습니다: {nan_cols}")
        ...
        for i, row in df.iterrows():
            ...
            sensor = {
                ...
                "hf_norm": float(row["hf_norm"]),
                "arc_flag": float(row["arc_flag"]),
                "arc_detect": float(row["arc_detect"]),
            ...
            publish_mqtt_status(device, sensor, pred, history_item)
            ...
            pending_db_rows.append(
                SensorData(
                    ...
                    hf_norm=sensor["hf_norm"],
                    arc_flag=sensor["arc_flag"],
                    arc_detect=sensor["arc_detect"],
